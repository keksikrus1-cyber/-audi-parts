# AudiParts — Агрегатор запчастей Audi

Веб-платформа для поиска и сравнения цен на запчасти Audi от нескольких поставщиков (Exist.ru, Emex.ru, Autodoc, Avtoall.ru).

## Активные части проекта

- `web` — React-приложение (основной фронтенд)
- `backend` — Bun + Hono + Prisma API
- `packages/contracts` — Shared Zod-схемы

## Отложенные части

- `mobile` — мобильное приложение (Expo/React Native) — **отложено**
- `landing` — маркетинговая страница (Astro) — **отложено**
- Production-деплой — **отложено**
- Реальные интеграции с поставщиками — **отложено**
- Реальные платежи ЮKassa — **отложено**

## Дизайн

**Источник правды:** `docs/design/audi-parts-reference.html`

Этот HTML-файл определяет все визуальные решения. Не менять цвета, отступы, типографику и компоненты без явного разрешения. Подробнее: `docs/DESIGN_REFERENCE.md`.

## Документация

- `MVP_PLAN.md` — цель, этапы, сущности, API-модули
- `docs/DESIGN_REFERENCE.md` — правила работы с дизайном
- `docs/UI_COMPONENT_MAP.md` — карта HTML → React компоненты
- `docs/LOCAL_DATABASE.md` — локальная PostgreSQL через Docker Compose

---

<!-- Оригинальная документация шаблона vibe ниже -->
